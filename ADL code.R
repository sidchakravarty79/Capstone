...
noise_fil<- function (i) {
  (-14.709) + (((i)/63) * (2*14.709))            ## Noise Filtering
}

median_fil<- function (i) {
medianFilter(i,windowSize = 3)                  ## Median Filtering
}
scaling <-function(x) {
  (x - min(x))/diff(range(x))                   ## Data scaling function
}

Sum_act<-data.frame()
for (i in 1:828) {
names<-noquote(unlist(strsplit(hmpfiles[i],"[/,.-]")))            ## Reading data sets
...
#Plot data after scaling and data balancing.
...
ggplot(aes(x = VM,fill=Act),data = files_dcs) + geom_histogram(binwidth=0.05,na.rm = T) + 
  ...
  theme(panel.grid.major = element_line(colour = "white")) + theme(panel.border = element_rect(linetype = "solid", 
                                                                                               colour = "white"))
# Split data into Train and Test groups for SVD and VD models
